...
type Cat = { id: number; nombre: string; slug: string }
...
async function cargar() {
  ...
  try {
    items.value = await apiFetch<Cat[]>('/catalogo/categorias/')
  } catch (e) { errorMsg.value = e instanceof Error ? e.message : 'Error' }
  finally { cargando.value = false }
}

function iniciarEditar(c: Cat) { editId.value = c.id; editForm.value = { nombre: c.nombre, slug: c.slug } }
function cancelarEditar() { editId.value = null }

async function guardarEditar(c: Cat) {
  const token = getAccessToken(); if (!token) return
  guardando.value = true; errorMsg.value = ''
  try {
    const u = await apiFetchAuth<Cat>(`/catalogo/admin/categorias/${c.id}/`, token, { method: 'PATCH', body: JSON.stringify(editForm.value) })
...
async function eliminar(c: Cat) {
  ...
  errorMsg.value = ''
  try {
    await apiFetchAuth<void>(`/catalogo/admin/categorias/${c.id}/`, token, { method: 'DELETE' })
...
async function crear() {
  ...
  try {
    const c = await apiFetchAuth<Cat>('/catalogo/admin/categorias/', token, { method: 'POST', body: JSON.stringify(newForm.value) })
...
<template>
  <div class="p-6 lg:p-10 max-w-2xl">
    <div class="mb-8">
      ...
      <h2 class="text-2xl font-black uppercase tracking-tight" style="font-family:'Playfair Display',serif;">Categorías</h2>
    </div>
    ...
    <div v-else class="bg-white/5 border border-white/[0.07] rounded mb-6 overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-white/[0.05]">
          <tr v-for="c in items" :key="c.id" class="hover:bg-white/[0.03] transition-colors">
            <td class="px-4 py-3">
              <input v-if="editId === c.id" v-model="editForm.nombre" class="admin-input" />
              <span v-else class="font-semibold">{{ c.nombre }}</span>
            </td>
            <td class="px-4 py-3">
              <input v-if="editId === c.id" v-model="editForm.slug" class="admin-input" />
              <span v-else class="text-white/40 text-[12px]">/{{ c.slug }}</span>
            </td>
            <td class="px-4 py-3">
              <div class="flex items-center gap-1">
                <template v-if="editId === c.id">
                  <button @click="guardarEditar(c)" :disabled="guardando" class="p-1.5 text-[#f5d984] hover:bg-white/10 rounded transition">
                    <Check class="h-3.5 w-3.5" />
                  </button>
                  <button @click="cancelarEditar" class="p-1.5 text-white/40 hover:bg-white/10 rounded transition">
                    <X class="h-3.5 w-3.5" />
                  </button>
                </template>
                <template v-else>
                  <button @click="iniciarEditar(c)" class="p-1.5 text-white/40 hover:text-[#f5d984] transition"><Pencil class="h-3.5 w-3.5" /></button>
                  <button @click="eliminar(c)" class="p-1.5 text-white/40 hover:text-red-300 transition"><Trash2 class="h-3.5 w-3.5" /></button>
                </template>
              </div>
            </td>
          </tr>
          <tr v-if="!items.length">
            <td colspan="3" class="px-4 py-8 text-center text-[11px] text-white/30">Sin categorías.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="bg-white/5 border border-white/[0.07] rounded p-5">
      <p class="text-[10px] font-black tracking-widest uppercase text-white/40 mb-4">Nueva categoría</p>
      <div class="flex gap-3 flex-wrap">
        <div class="flex-1 min-w-[140px]">
          <label class="admin-label">Nombre *</label>
          <input v-model="newForm.nombre" type="text" class="admin-input" placeholder="Monturas" />
        </div>
        <div class="flex-1 min-w-[140px]">
          <label class="admin-label">Slug</label>
          <input v-model="newForm.slug" type="text" class="admin-input" placeholder="monturas" />
        </div>
